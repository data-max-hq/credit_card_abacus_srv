    ...
    def do_load(self):
        ...
        try:
            if AbacusCCLoaderFromCentaur.is_load_fin():
                self.logger.info("Abacus Finished for max working day, Starting Delinquency...")

                if AbacusCCLoaderFromCentaur.is_ok_to_call_deliquency():
                    self.logger.info("Abacus Finished for this day, Starting Delinquency...")
                    
                    session = Session()

                    transport = Transport(session=session, timeout=900)  # Timeout of 15 minutes
                    ...
                    if centaur_service.ProcessDeliquency():
                        ...
                        try:
                            ...
                            if load_status == AbacusCCLoaderFromCentaur.LoadStatus.WAITING_FOR_FIN:
                                self.logger.info("Waiting for fin! Trying again later.")
                            elif load_status == AbacusCCLoaderFromCentaur.LoadStatus.SUCCESS:
                                self.logger.info("Done loading CreditCard data into Abacus.")
                            elif load_status == AbacusCCLoaderFromCentaur.LoadStatus.ERROR:
                                self.logger.error("Error loading CreditCard data into Abacus.")
                            elif load_status == AbacusCCLoaderFromCentaur.LoadStatus.FINISHED:
                        ...
                        self.logger.info("Delinquency not finished! Trying again later.")
                else:
                    self.logger.info("Waiting for Abacus for today’s load! Trying again later.")
            else:
