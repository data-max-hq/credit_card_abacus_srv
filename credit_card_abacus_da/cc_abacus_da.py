...
class CC_AbacusDA:
    def __init__(self):
        self.abacus_connection = os.getenv("AbacusConStr")  # Environment variable or config management
        self.schema_used = os.getenv("SchemaUsed")  # Environment variable or config management
    ...
    def set_not_cc_working_day(self):
        ...
        try:
            ...
            row = cursor.fetchone()
            if row:
                self.NotCCWorkingDay = datetime.strptime(row[0], '%Y-%m-%d')  # assuming date format is Y-m-d
    ...
    def handle_working_day(self, centaur_working_day):
        query = f"SELECT * FROM {self.schema_used}.w02_ccworking_day WHERE working_day = %s"
        cursor = self.conn.cursor()
        ...
        for index, row in atmp_t17.iterrows():
            ...
            if int(row["DAYS_PAST_DUE"]) > 0:
                ...
                try:
                    ...
                    self.build_data_table_atmp_t18_for_single_product(atmp_t18, t18_rows, row)
                except Exception as ex:
    ...
    def get_atmp_t18(self):
        query = f"SELECT * FROM {self.schema_used}.atmp_t18_cc_payment_schedule WHERE 1<>1"
    ...
    def build_data_table_atmp_t18_for_single_product(self, atmp_t18, t18rows, atmp_t17_row):
        ...
        if not t18rows.empty:
            for _, t18row in t18rows.iterrows():
                ...
                atmp_t18_row["PRINCIPAL_PAYMENT_AMOUNT"] = t18row["PRINCIPAL_PAYMENT_AMOUNT"]
                atmp_t18_row["PRINCIPAL_PAYMENT_DATE"] = t18row["PRINCIPAL_PAYMENT_DATE"]
                atmp_t18_row["INTEREST_PAYMENT_DATE"] = t18row["INTEREST_PAYMENT_DATE"]
                atmp_t18_row["INTEREST_PAYMENT_AMOUNT"] = t18row["INTEREST_PAYMENT_AMOUNT"]
                ...
                atmp_t18_row["PENALTY_INTEREST_AMOUNT"] = t18row["PENALTY_INTEREST_AMOUNT"]
                ...
                if int(t18row["PERIOD"]) == int(atmp_t17_row["PERIOD"]):
                    ...
                    atmp_t18_row["LAST_SUM_OF_PAYMENT"] = float(t18row["LAST_SUM_OF_PAYMENT"])
                    atmp_t18_row["IS_PASTDUE"] = "1"
                    nr_payments_past_due += 1
    ...
    def truncate_atmp_t17_dpd_credit_cards(self, is_service):
        try:
            query = f"TRUNCATE TABLE {self.schema_used}.atmp_t17_dpd_credit_cards" if is_service else f"DELETE FROM {self.schema_used}.atmp_t17_dpd_credit_cards"
    ...
    def truncate_atmp_t18_cc_payment_schedule(self, is_service):
        try:
            query = f"TRUNCATE TABLE {self.schema_used}.atmp_t18_cc_payment_schedule" if is_service else f"DELETE FROM {self.schema_used}.atmp_t18_cc_payment_schedule"
            with self.conn.cursor() as cursor:
                cursor.execute(query)
            self.conn.commit()
            return True
    ...
    def bulk_insert_atmp_t17(self, atmp_t17: pd.DataFrame):
        ...
        try:
            ...
            return self.bulk_insert_abacus(atmp_t17, "atmp_t17_dpd_credit_cards", column_names_atmp_t17, column_names_atmp_t17)
        except Exception as ex:
    ...
    def bulk_insert_atmp_t18(self, atmp_t18: pd.DataFrame):
        ...
        try:
            # Assuming BulkInsertAbacus is another method for bulk insertion, we'll call it here.
            return self.bulk_insert_abacus(atmp_t18, "atmp_t18_cc_payment_schedule", column_names_atmp_t18, column_names_atmp_t18)
    ...
    def bulk_insert_abacus(self, source_table: pd.DataFrame, destination_table_name: str, source_columns: list, destination_columns: list):
        ...
        if source_columns and destination_columns:
            if len(source_columns) == len(destination_columns) and len(source_columns) > 0:
                # Align columns in the DataFrame to match the destination columns
                source_table = source_table[source_columns]
                source_table.columns = destination_columns  # Rename columns to match destination
        ...
        try:
            ...
            self.conn.rollback()
            raise Exception(f"Bulk insert into {destination_table_name} failed, reason: {str(ex)}")
        finally:
            cursor.close()
            self.conn.close()
